...
for suffix in ['', '_500m']:
	...
	for (sp, marker) in zip(discordant_species, 'dDs<>^v'):
		...
		Y = [r['Tk18'+suffix] for r in H]
		eX = [1.96 * r['SE_Tiso_species'+suffix] for r in H]
		eY = [1.96 * r['SE_Tk18'+suffix] for r in H]
	...
	Y = [r['Tk18'+suffix] for r in G]
	eX = [1.96 * r['SE_Tiso_species'+suffix] for r in G]
	eY = [1.96 * r['SE_Tk18'+suffix] for r in G]
	...
	savefig(f'plots/Tiso_vs_Tk18_planktic{suffix}.pdf')
	close(fig)
	...
	for ax, T, txt in [
		...
		(subplot(122), 'Tk18', 'With atlas temperature estimates\nfor discordant data points'),
		]:
